...
| 靶机/虚拟机实验环境 / Proxmox VE / PVE | `pve-lab/` — PVE MCP 编排（VM ID） | 与 windbg-reverse 组合做内核调试 |
...
| "给我快照一台靶机投样本" / "编排一个实验网络" / "PVE / Proxmox / VM ID" | `pve-lab/SKILL.md` — PVE 靶机编排 + 内核调试组合 |
...
| qm / pve-mcp / Proxmox API | `pve-lab/` — PVE VM 编排与快照 |
